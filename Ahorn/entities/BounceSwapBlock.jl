module BounceHelperBounceSwapBlock

using ..Ahorn, Maple

@mapdef Entity "BounceHelper/BounceSwapBlock" BounceSwapBlock(x::Integer, 
                                                              y::Integer, 
                                                              width::Integer=16, 
                                                              height::Integer=16,
                                                              moon::Bool=false) 
...
const placements = Ahorn.PlacementDict(
    "Bounce Swap Block (Bounce Helper)" => Ahorn.EntityPlacement(
        BounceSwapBlock,
        "rectangle",
        Dict{String, Any}(),
        swapFinalizer
...
Ahorn.nodeLimits(entity::BounceSwapBlock) = 1, 1

Ahorn.minimumSize(entity::BounceSwapBlock) = 16, 16
Ahorn.resizable(entity::BounceSwapBlock) = true, true

function Ahorn.selection(entity::BounceSwapBlock)
    x, y = Ahorn.position(entity)
...
function getTextures(entity::BounceSwapBlock)
    moon = get(entity, "moon", false)
...
function Ahorn.renderSelectedAbs(ctx::Ahorn.Cairo.CairoContext, entity::BounceSwapBlock, room::Maple.Room)
...
function Ahorn.renderAbs(ctx::Ahorn.Cairo.CairoContext, entity::BounceSwapBlock, room::Maple.Room)
    sprite = get(entity.data, "sprite", "block")
