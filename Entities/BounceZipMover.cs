...
namespace Celeste.Mod.BounceHelper {
    [CustomEntity("BounceHelper/BounceZipMover")]
    [Tracked]
    class BounceZipMover : Solid {
		private class ZipMoverPathRenderer : Entity {
			public BounceZipMover BounceZipMover;
			...
			public ZipMoverPathRenderer(BounceZipMover zipMover) {
				base.Depth = 5000;
				BounceZipMover = zipMover;
				from = BounceZipMover.start + new Vector2(BounceZipMover.Width / 2f, BounceZipMover.Height / 2f);
				to = BounceZipMover.target + new Vector2(BounceZipMover.Width / 2f, BounceZipMover.Height / 2f);
			...
			public override void Render() {
				...
				if (BounceZipMover.drawBlackBorder) {
					Draw.Rect(new Rectangle((int)(BounceZipMover.X + BounceZipMover.Shake.X - 1f), (int)(BounceZipMover.Y + BounceZipMover.Shake.Y - 1f), (int)BounceZipMover.Width + 2, (int)BounceZipMover.Height + 2), Color.Black);
				}
			}
			...
			private void DrawCogs(Vector2 offset, Color? colorOverride = null) {
				...
				float rotation = BounceZipMover.percent * (float)Math.PI * 2f;
				...
				for (float num = 4f - BounceZipMover.percent * (float)Math.PI * 8f % 4f; num < (to - from).Length(); num += 4f) {
		...
		private Vector2 zipLiftSpeed = Vector2.Zero;
		...
		public BounceZipMover(Vector2 position, int width, int height, Vector2 target, bool moon)
			...
			if (moon) {
				bloom.Position = new Vector2(base.Width / 2f, 10f);
			} else {
		...
		public BounceZipMover(EntityData data, Vector2 offset)
		...
		private IEnumerator Sequence() {
			...
			while (true) {
				...
				triggered = true;
				sfx.Play(moon ? "event:/new_content/game/10_farewell/zip_mover" : "event:/game/01_forsaken_city/zip_mover");
				...
				while (at2 < 1f) {
					...
					MoveTo(to);
					zipLiftSpeed = LiftSpeed;
					yield return null;
				}
				zipLiftSpeed = Vector2.Zero;
				StartShaking(0.2f);
